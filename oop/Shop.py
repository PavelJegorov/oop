...
class Product:
    ...
    def __eq__(self, other):
        return insintance(other, Product) and other.id == self.id
...
class Shop:
    ...
    def add_product(self,product: Product, count: int = 1) -> bool:
         ...
         if product.id not in self.products:
            ...
            self.product_count[product.id] += count
        return True
    ...
    def move_to_cart(self, product: Product, count=1) -> bool:
        if product.id not in self.products:
            return False
        if count > self.product_count[product.id]:
            return False
        self.product_count[product.id] -= count
        return True
...
class Cart:
    ...
    def get_total_price(self):
        total_price =0
        for pid, count in self.product.items():
...
shop1.add:product(p1, 10)
shop2.add:product(p2, 10)
